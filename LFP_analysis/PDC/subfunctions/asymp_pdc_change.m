   ...
   for i = 1:nChannels,
      for j = 1:nChannels,
         ...
         if alpha ~= 0,
            ...
            ic1(i, j, ff) = pdc(i, j, ff) - sqrt(varass1(i, j, ff))*icdf('norm',1-alpha/2.0,0,1);
            ic2(i, j, ff) = pdc(i, j, ff) + sqrt(varass1(i, j, ff))*icdf('norm',1-alpha/2.0,0,1);
...
if alpha ~= 0
   ...
   c.patdenr = patdenr(:,:,1:140);
   c.patdfr = patdfr(:,:,1:140);
else
   c.pdc=pdc(:,:,1:140);
   c.metric=metric;
   c.alpha=0;
