...
%   This implementation is a FORTRAN code translation made by KS from 
...
while 1,
   ...
   %  Update residues
   %
   % existe erro no cálculo dos residuos
   %
   Tef=ef;
   %ef(1:NUMCHS,M+1:N)=Tef(:,M+1:N)+AM*eb(:,M:N-1);
   %eb(1:NUMCHS,M+1:N)=eb(:,M:N-1)+BM*Tef(:,M+1:N);
